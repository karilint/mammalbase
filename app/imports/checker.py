"""All checks defined in this file
"""
import re
import numpy
import pandas as pd
...
class Check:
    ...
    def check_valid_author(self, df):
        ...
        for author in (df.loc[:, 'author']):
            ...
            if author == "nan":
                messages.error(
                    self.request, "The author is empty at row " + str(counter) + ".")
                return False
    ...
    def check_headers_ets(self, df):
        import_headers = list(df.columns.values)
        accepted_headers = ['references', 'verbatimScientificName',
                            'taxonRank', 'verbatimTraitName', 'verbatimTraitUnit', 'author']
                ...
                for row, value in enumerate(df.loc[:, header]):
                    if optional_headers[header] != str:
                        if isinstance(value, str):
                            value = value.replace(",", ".")
                        try:
                            df.loc[row, header] = optional_headers[header](
                                value)
                        except ValueError:
                            messages.error(
                                self.request,
                                f"The {header} on row {row+1} is an incorrect type. ",
                                f"It should be {type_names[optional_headers[header]]}."
                            )
                            return False
        return True

    def _calculate_nfe(self, row):
        value_sum = sum([possible_nan_to_zero(row[value]) for value in row.keys() if (
            "verbatimTraitValue" in value and not "nitrogen_free_extract" in value)])
        return min(
            abs(value_sum - 100),
            abs(value_sum - 1000)
        )
    ...
    def check_nfe(self, df):
        ...
        if 'verbatimTraitValue__nitrogen_free_extract' in headers:
            ...
            mask[mask.notnull()] = ""
            new_mm = df['measurementMethod__nitrogen_free_extract'].fillna(
                mask.fillna(missing_nfe_message))
            new_mm.replace(r'^$', numpy.nan, regex=True, inplace=True)
            df["measurementMethod__nitrogen_free_extract"] = new_mm
            df["verbatimTraitValue__nitrogen_free_extract"].fillna(
                df.apply(self._calculate_nfe, axis=1), inplace=True)
        else:
            df["measurementMethod__nitrogen_free_extract"] = missing_nfe_message
            df["verbatimTraitValue__nitrogen_free_extract"] = df.apply(
                self._calculate_nfe, axis=1)
    ...
    def _check_if_plant(self, name):
        ...
        try:
            kingdom = rank_id[max(
                rank_id)]['data'][0]['results'][0]['classification_path'].split('-')[0]
        except ValueError:
            return None
    ...
    def check_author(self, df):
        for row, author in enumerate(df.loc[:, 'author'], 1):
            if len(str(author)) != 19:
                messages.error(
                    self.request,
                    f"The author \'{author}\' on row {row} is not in the correct form."
                )
                return False
            if "X" in author:
    ...
    def check_verbatimScientificName(self, df, taxon_rank_included=True):
        for row, name in enumerate(df.loc[:, 'verbatimScientificName'], 1):
            if name == "nan" or pd.isna(name):
                messages.error(
                    self.request, f"Scientific name \'{name}\' is empty at row {row}.")
                return False
            if len(name) > 250:
                messages.error(
                    self.request, f"Scientific name \'{name[:10]}...\' is too long at row {row}.")
                return False
        ...
        for row, item in enumerate(df_new.values, 1):
            ...
            if (
                len(names_list) > 3 and not
                any(x in {"sp.", "sp", "cf.", "cf", "indet.", "indet", "aff.", "aff", "spp.", "spp"}
                    for x in names_list
                )
                ):
                messages.error(
                    self.request,
                    f"Scientific name \'{str(item[0])}\' is not in the correct format on row {row}."
                )
                return False
            if (
                len(names_list) == 3 and item[1] not in
                {'Subspecies', 'subspecies'} and not
                any(
                ...
                messages.error(
                    ...
                    f"or taxonomic rank \'{str(item[1])}\' should be 'Subspecies' on row {row}.")
                return False
            if (
                len(names_list) == 2 and item[1] not in
        ...
        if "measurementValue" in import_headers:
            for row, value in enumerate(df.loc[:, 'measurementValue'], 1):
                ...
                if value <= 0:
                    messages.error(
                        self.request,
                        f"The measurement value on row {row} needs to be bigger than zero."
                    )
                    return False
    ...
    def check_references(self, df, force: bool):
        for row, ref in enumerate(df.loc[:, 'references'], 1):
            ...
            if len(ref) < 10 or len(ref) > 500:
                messages.error(
                    self.request, f"Reference is too short or too long on row {row}.")
                return False
            match = re.match(r'.*([1-2][0-9]{3})', ref)
            if match is None:
                messages.error(
                    self.request, f"Reference does not have a year number on row {row}.")
                return False
        return True
            ...
            if header in import_headers:
                for row, value in enumerate(df.loc[:, header], 1):
                    if len(str(value)) > all_headers[header]:
                        messages.error(
                            self.request, f"{header} is too long on row {row}.")
                        return False
        return True
    ...
    def check_min_max(self, df):
        ...
        if "measurementValue_min" in import_headers:
            ...
            if "measurementValue_min" not in import_headers:
                messages.error(
                    self.request, "There should be a header called 'measurementValue_min'.")
                return False
        ...
        if "verbatimTraitValue" in import_headers:
            counter = 1
            df_new = df[['measurementValue_min',
                         'measurementValue_max', 'verbatimTraitValue']]
