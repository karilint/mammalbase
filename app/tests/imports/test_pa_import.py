...
class ProximateAnalysisImporterTest(TestCase):
    def setUp(self):
        ...
        self.accountuser = SocialAccount.objects.create(uid='1111-1111-2222-2223', user_id=self.user.pk)
    ...
    def test_import_pa_post_correct_file(self):       
        ...
        self.assertEqual(str(messages[0]), "File imported successfully. 1 rows of data were imported.")
        self.assertEqual(response.status_code, 302)

    @skip # FIX: self.assertEqual(len(messages), 1), AssertionError: 3 != 1
    ...
    @skip # FIX: same than ^^^ ??? self.assertEqual(len(messages), 1), AssertionError: 3 != 1
    def test_import_pa_post_incorrect_file(self):       
