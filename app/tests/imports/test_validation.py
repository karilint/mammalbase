...
class ValidationTest(TestCase):
    ...
    def test_choicevalue(self):
        ...
        gender, created = ChoiceValue.objects.get_or_create(
            choice_set="Gender", caption="Male")

        if created == False:
            self.fail(
                "Exception in creating MOC data (ChoiceValue: Gender and Male)")

        data = {"sex": "male"}
        errors = self.instance.validate_choice_value(
            data, "sex", "choiceValue:gender")
        ...
        errors = self.instance.validate_choice_value(
            data, "sex", "choiceValue:gender")
        self.assertEqual(
            errors[0], "'polkuauto' is invalid value for sex field")
    ...
    def test_validate_author_fields(self):
        ...
        # Valid format with non-numeric characters
        data = {'author': 'abcd-efgh-ijkl-mnop'}
        errors = self.instance.validate_author_fields(data, 'author')
        self.assertEqual(errors, [author_error])
    ...
    def test_choice_value(self):
        gender, created = ChoiceValue.objects.get_or_create(
            choice_set="Gender", caption="Male")
        lifestage, created = ChoiceValue.objects.get_or_create(
            choice_set="Lifestage", caption="Adult")
    ...
    def test_coordinate_format(self):
        ...
        self.assertEqual(self.instance.coordinate_format(
            coords, test_dict), "decimal degrees")

        latitude = "12°34.567'N"
        longitude = "78°90.123'W"
        coords = f"{latitude}, {longitude}"
        self.assertEqual(self.instance.coordinate_format(
            coords, test_dict), "degrees minutes")

        latitude = "12°34'56.789\" N"
        longitude = "78°90'12.345\" W"
        coords = f"{latitude}, {longitude}"
        self.assertEqual(self.instance.coordinate_format(
            coords, test_dict), "degrees decimals")
        self.assertEqual(self.instance.coordinate_format(
            "32S 485146mE 4037735mN", test_dict), "UTM")
        self.assertEqual(self.instance.coordinate_format(
            "Invalid Coordinate", test_dict), "No match found")
    ...
    def test_validate_coordinateSystem_fields(self):
        ...
        self.assertNotEqual(self.instance.validate_coordinateSystem_fields(
            mock_data, "verbatimCoordinateSystem", rule), [])
        self.assertEqual(self.instance.validate_coordinateSystem_fields(
            mock_data2, "verbatimCoordinateSystem", rule), [])
        self.assertEqual(self.instance.validate_coordinateSystem_fields(
            mock_data3, "verbatimCoordinateSystem", rule), [coord_error])
        self.assertNotEqual(self.instance.validate_coordinateSystem_fields(
            mock_data3, "verbatimCoordinateSystem", rule), [])
