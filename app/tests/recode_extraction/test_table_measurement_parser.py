...
class TableMeasurementParserTests(SimpleTestCase):
    def test_extracts_skull_and_external_rows(self):
        ...
        self.assertIn(('Mus pahari pahari', 'Body Mass (BW)'), names)
        self.assertIn(('Mus pahari jacksoniae', 'Body Mass (BW)'), names)
        self.assertIn(('Mus pahari gairdneri', 'Greatest Length of Skull (GLS)'), names)
    ...
    def test_dedupes_records(self):
        ...
        records = extract_trait_records_from_measurement_tables([table], abbr_dict={})
        uniq = {
            (r['verbatimScientificName'], r['verbatimTraitName'], r['statisticalMethod'], r['verbatimTraitValue'], r['measurementValue_min'], r['measurementValue_max'])
            for r in records
        }
        self.assertEqual(len(records), len(uniq))

    def test_combines_mean_sd_and_range_in_same_row(self):
        table = (
            "M. p. pahari (N = 33) M. p. jacksoniae (N = 22) M. p. gairdneri (N = 5) "
        ...
        target = [
            ...
            and r['verbatimTraitName'] == 'Greatest Length of Skull (GLS)'
        ][0]
        self.assertEqual(target['verbatimTraitValue'], '21.48')
        self.assertEqual(target['measurementValue_min'], 21.48)
        self.assertEqual(target['measurementValue_max'], 21.48)
        self.assertEqual(target['statisticalMethod'], 'mean, range')
