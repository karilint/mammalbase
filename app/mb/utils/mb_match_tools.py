...
def match_gbif(name, **kwargs):
    ...
    response = requests.get("https://api.gbif.org/v1/species/match", params=parameters, timeout=30)
        ...
        if data["status"] == 'ACCEPTED':
            ...
            if data["acceptedUsageKey"]:
                accepted = requests.get(
                    "https://api.gbif.org/v1/species/"+str(data["acceptedUsageKey"]),
                    timeout=30
                )
