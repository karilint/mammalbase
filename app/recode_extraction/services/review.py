...
def apply_assertion_review(
    assertion: ExtractedAssertionModel,
    *,
    reviewer,
    review_status: str,
    edited_value: str = '',
...
def persist_approved_assertions_to_ets(run: SourceExtractionRun):
    ...
    approved = run.assertions.filter(
        review_status=ExtractedAssertionModel.ReviewStatus.APPROVED,
        ets_persisted=False,
    )
...
def export_assertions_csv(run: SourceExtractionRun, queryset=None) -> str:
    ...
    writer.writerow([
        'id', 'subject_taxon', 'trait_name', 'value_raw', 'edited_value', 'unit', 'edited_unit',
        'confidence', 'page_number', 'review_status', 'ets_persisted', 'mapped_trait_id', 'unmapped_reason', 'context'
    ])
