...
class LocationAPI:
    ...
    def get_nature_reserves(self, location_name):
        ...
        data = response.json()
        if 'geonames' in data:
            nature_reservations = [place for place in data['geonames'] if place.get('fcode') == 'RESN' or place.get('fcode') == 'RESW' or place.get('fcode') == 'RESF' or place.get('fcode') == 'PRK' or place.get('fcode') == 'RES']
