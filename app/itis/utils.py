...
def generate_standard_values_pa(items):
    ...
    for item in items.keys():
        if "reported" in item and "dm" not in item and (
            "moisture" not in item and items[item] is not None):
    ...
    for item in list(items.keys()):
        if "reported" not in item or "dm" in item or "moisture" in item:
            ...
            standard_items[item.replace("reported","std")] =(
                ((Decimal(items[item]) / Decimal(10)) / item_sum) * Decimal(100))
        else:
            standard_items[item.replace("reported","std")] = (
                (Decimal(items[item]) / item_sum) * Decimal(100))
...
def create_return_data(tsn, scientific_name, status='valid'):
    ...
    if status in {'valid', 'accepted'}:
        ...
        classification_path = hierarchyToString(scientific_name,
                                                hierarchy,
                                                'hierarchyList',
                                                'taxonName')
        classification_path_ids = hierarchyToString(tsn,
                                                    hierarchy,
                                                    'hierarchyList',
                                                    'tsn',
                                                    stop_index=classification_path.count("-"))
        classification_path_ranks = hierarchyToString('Species',
                                                      hierarchy,
                                                      'hierarchyList',
                                                      'rankName',
                                                      stop_index=classification_path.count("-"))
...
def create_tsn(results, tsn):
    ...
    if len(taxonomic_unit)==0:
        ...
        taxonomic_unit = TaxonomicUnits(tsn=tsn,
    ...
    if results['data'][0]['results'][0]['taxonomic_status'] in ("invalid", "not accepted"):
        ...
        accepted_taxonomic_unit = create_tsn(accepted_results,
                                             int(accepted_results['data'][0]['results'][0]['taxon_id']))
        sl_qs = SynonymLinks.objects.all().filter(tsn = tsn)
        if len(sl_qs) == 0:
            sl = SynonymLinks(tsn = taxonomic_unit,
